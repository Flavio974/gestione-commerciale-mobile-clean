...
class SmartAssistant {
  ...
  async startRecording() {
    ...
    try {
      ...
      try {
        console.log('🔍 DEBUG: Chiamando showRecordingUI()...');
        this.showRecordingUI();
        console.log('🔍 DEBUG: showRecordingUI() completato');
        
        console.log('🔍 DEBUG: Avvio timer - metodo esiste?', typeof this.startRecordingTimer);
        console.log('🔍 DEBUG: This context:', this.constructor.name);
        this.startRecordingTimer();
        console.log('🔍 DEBUG: startRecordingTimer() chiamato con successo');
      } catch (uiError) {
        console.error('❌ Errore in UI/Timer setup:', uiError);
        // Prova comunque ad avviare il timer anche se l'UI fallisce
        try {
          this.startRecordingTimer();
          console.log('🔍 Timer avviato nonostante errore UI');
        } catch (timerError) {
          console.error('❌ Errore anche nel timer:', timerError);
        }
      }
   ...
   */
  stopRecording() {
    console.log('🛑 === STOP RECORDING CHIAMATO ===');
    console.log('🛑 MediaRecorder:', !!this.mediaRecorder);
    console.log('🛑 IsRecording:', this.isRecording);
    
    if (this.mediaRecorder && this.isRecording) {
      console.log('🛑 Fermando MediaRecorder...');
      ...
      console.log('🛑 === STOP RECORDING COMPLETATO ===');
    } else {
      console.log('⚠️ STOP RECORDING: Condizioni non soddisfatte');
    }
  }
  ...
  startRecordingTimer() {
    ...
    console.log('🕐 Timer registrazione avviato:', new Date().toLocaleTimeString());
    ...
    this.recordingTimer = setInterval(() => {
      ...
      if (elapsed % 5 === 0) {
        console.log(`🕐 Timer: ${elapsed}s (${minutes}:${seconds})`);
      }
      ...
      // 🛑 AUTO-STOP: Ferma automaticamente dopo 30 secondi
      if (elapsed >= 30) {
        console.log('⏰ TIMEOUT RAGGIUNTO! Elapsed:', elapsed, 'seconds');
        console.log('⏰ Fermando registrazione automaticamente...');
