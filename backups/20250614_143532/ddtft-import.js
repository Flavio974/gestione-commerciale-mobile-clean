...
const DDTFTImport = {
    ...
    if (detectedType === 'DDT') {
      ...
      try {
        ...
        if (mappedResult.clientName) {
          const parts = mappedResult.clientName.split(/\s+/);
          const uniqueParts = [];
          let lastPart = '';
  ...
  extractFromDeliverySection: function(text) {
    ...
    if (deliveryMatch) {
      ...
      // Cerca l'indirizzo completo (potrebbero essere più righe)
      let address = '';
      ...
      if (address && this.validateDeliveryAddress(address)) {
        console.log("✅ Indirizzo trovato da sezione consegna:", address);
        return address;
      }
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inTable) {
        ...
        if (match) {
          ...
          const price = match[5].replace(',', '.');
          const discount = match[6].replace(',', '.');
          const amount = match[7].replace(',', '.'); // QUESTO È L'IMPORTO CORRETTO!
