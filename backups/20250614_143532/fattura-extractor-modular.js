...
class FatturaExtractorModular extends FatturaExtractor {
    ...
    extractClientName() {
        // Prima cerca nella parte finale del documento (dopo VETTORI)
                ...
                if (line.match(/^[A-Z][A-Z\s\.&]+/) && line.length > 5) {
                    ...
                    if (i < lines.length - 1) {
                        ...
                        if (nextLine.match(/^di[A-Z]/)) {
                            ...
                            this.log(`✅ Cliente trovato (formato Alfieri FT): ${fullName}`);
                            return fullName;
                        }
                    }
                    // Altrimenti usa solo questa riga
                    this.log(`✅ Cliente trovato (formato Alfieri FT): ${line}`);
                    return line;
                }
            }
        }
